...
const ExportRequest = {
  ...
  async complete(requestId, exportItems) {
    ...
    try {
      ...
      const [[exportReq]] = await conn.query(
        'SELECT id, status FROM export_requests WHERE id = ?',
        [requestId]
      );
      if (!exportReq) throw { status: 404, message: `Không tìm thấy yêu cầu #${requestId}` };
      if (exportReq.status !== 'APPROVED') {
        throw { status: 400, message: `Yêu cầu #${requestId} không ở trạng thái APPROVED (hiện tại: ${exportReq.status})` };
      }
      ...
      // ── UC6: Kiểm tra tồn kho dưới min_stock → tạo đề xuất mua hàng ──
      const requisitions = [];
      for (const medId of Object.keys(exportedPerMed)) {
        try {
          const [[stockRow]] = await conn.query(
            `SELECT m.id, m.name, m.min_stock, m.max_stock,
                    COALESCE(SUM(b.quantity), 0) AS current_stock
             FROM medicines m
             LEFT JOIN batches b ON b.medicine_id = m.id AND b.quantity > 0
             WHERE m.id = ?
             GROUP BY m.id, m.name, m.min_stock, m.max_stock`,
            [medId]
          );
          if (!stockRow || stockRow.min_stock <= 0) continue;

          const currentStock = Number(stockRow.current_stock);
          if (currentStock <= stockRow.min_stock) {
            // Tránh tạo trùng nếu đã có PENDING cho cùng medicine
            const [[existing]] = await conn.query(
              `SELECT id FROM purchase_requisitions
               WHERE medicine_id = ? AND status = 'PENDING' LIMIT 1`,
              [medId]
            );
            if (existing) continue;

            const suggestedQty = Math.max(
              (stockRow.max_stock || 0) - currentStock,
              stockRow.min_stock - currentStock
            );

            await conn.query(
              `INSERT INTO purchase_requisitions
                 (medicine_id, medicine_name, current_stock, min_stock, suggested_qty, status)
               VALUES (?, ?, ?, ?, ?, 'PENDING')`,
              [medId, stockRow.name, currentStock, stockRow.min_stock, suggestedQty]
            );
            requisitions.push({ medicineName: stockRow.name, currentStock, suggestedQty });
          }
        } catch (_) { /* không block response nếu step này lỗi */ }
      }
      // ─────────────────────────────────────────────────────────────
      ...
      return {
        ...
        purchaseRequisitions: requisitions,   // danh sách đề xuất vừa tạo (có thể rỗng)
      };
    } catch (err) {
      await conn.rollback();
      throw err;
