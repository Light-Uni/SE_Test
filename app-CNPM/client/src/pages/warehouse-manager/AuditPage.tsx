...
import { getAudits, getAuditById, confirmAudit, exportAuditReport } from "../../api/medicineRequestApi";
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            {selectedSession && (
              <div ref={exportRef} style={{ position: "relative" }}>
                ...
                >
                  <Icon name={exportLoading ? "hourglass_empty" : "download"} size={16} />
                ...
                {showExportMenu && (
                  <div style={{
                    ...
                    {(["excel", "pdf"] as const).map((fmt) => (
                      <button
                        key={fmt}
                        id={`btn-export-audit-${fmt}`}
                        onClick={async () => {
                          setShowExportMenu(false);
                          setExportLoading(true);
                          try {
                            await exportAuditReport(selectedSession.id, fmt);
                          } catch {
                      ...
                      >
                        <Icon name={fmt === "excel" ? "table_chart" : "picture_as_pdf"} size={16}
                          style={{ color: fmt === "excel" ? "#059669" : "#DC2626" }} />
                        {fmt === "excel" ? "Xuất Excel (.xlsx)" : "Xuất PDF (.pdf)"}
                      </button>
                    ))}
                  </div>
                )}
